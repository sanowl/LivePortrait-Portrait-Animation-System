...
class CelebADataset:
    """
    Dataset class for CelebA, handling landmark extraction and transformations.
    """
    ...
    def __getitem__(self, idx) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        """
        Returns a tuple: (source_image, driving_image, source_landmarks, driving_landmarks).
        Images are JAX arrays and landmarks are NumPy arrays.
        """
    ...
    def extract_landmarks(self) -> list:
        """
        Extracts facial landmarks for all images in the dataset.
        Uses face_alignment library and handles cases where landmarks are not detected.
        """
        landmarks = []
        for item in tqdm(self.dataset, desc="Extracting landmarks"):
            ...
            if detected_landmarks is not None:
                ...
                landmarks.append(np.zeros((68, 2)))  # Placeholder for missing landmarks
        return landmarks

class AppearanceFeatureExtractor(nn.Module):
    """
    Extracts appearance features from the source image. 
    Uses convolutional layers followed by average pooling.
    """
    @nn.compact
    def __call__(self, x):
...
class MotionExtractor(nn.Module):
    ...
    Uses convolutional layers followed by a fully connected layer.
    """
    @nn.compact
    def __call__(self, x):
        x = nn.Conv(features=64, kernel_size=(7, 7), strides=(2, 2), padding="SAME")(x)
...
class WarpingModule(nn.Module):
    """
    Predicts the dense motion field (flow) based on appearance and motion features.
    """
    @nn.compact
...
class Decoder(nn.Module):
    """
    Generates the final output image from the warped features.
    Uses transposed convolutions to upsample the features.
    """
    @nn.compact
    def __call__(self, x):
...
class AttentionModule(nn.Module):
    """
    Self-attention module to refine the warped features.
    """
    @nn.compact
    def __call__(self, x):
        b, h, w, c = x.shape
...
class LivePortrait(nn.Module):
    """
    The main LivePortrait model that combines all the modules.
    """
    def setup(self):
    ...
    def warp(self, features, flow):
        """
        Warps the appearance features using the predicted flow.
        """
    ...
    def get_grid(b, h, w):
        """
        Generates a normalized grid for the warping operation.
        """
...
class VGGPerceptualLoss(nn.Module):
    """
    VGG19-based perceptual loss module.
    Calculates the difference in feature activations between the output and target images. 
    """
    def setup(self):
        self.blocks = [
            self.vgg_block(64, 2),
...
def perceptual_loss(vgg_apply_fn, vgg_params, output, target):
    """
    Calculates the perceptual loss using the VGG network.
    """
    output_features = vgg_apply_fn({'params': vgg_params}, output)
    target_features = vgg_apply_fn({'params': vgg_params}, target)
...
def stitching_loss(output, source):
    """
    Calculates the stitching loss to ensure smooth transitions at the boundaries. 
    """
...
def create_stitching_mask(shape):
    """
    Creates a mask for the stitching loss.
    """
    mask = jnp.ones(shape)
...
def eye_retargeting_loss(output, source_landmarks, driving_landmarks):
    """
    Calculates the eye retargeting loss to preserve eye movements.
    """
...
def extract_eye_landmarks(landmarks):
    """
    Extracts the landmarks corresponding to the eyes.
    """
...
def extract_eye_region(image):
    """
    Extracts the eye region from the image.
    """
...
def lip_retargeting_loss(output, source_landmarks, driving_landmarks):
    """
    Calculates the lip retargeting loss to preserve lip movements.
    """
...
def extract_lip_landmarks(landmarks):
    """
    Extracts the landmarks corresponding to the lips.
    """
    return landmarks[:, 48:]

def extract_lip_region(image):
    """
    Extracts the lip region from the image.
    """
    return image[:, :, 2*image.shape[2]//3:, image.shape[3]//4:3*image.shape[3]//4]

class TrainState(train_state.TrainState):
    """
    Custom TrainState to hold the discriminator and VGG parameters and apply functions.
    """
    discriminator_apply_fn: Any 
    vgg_apply_fn: Any          
    discriminator_params: Any  
    vgg_params: Any 
...
def train_step(apply_fn, state: TrainState, batch, rng):
    ...
    Calculates the loss and gradients, and updates the generator parameters.
    """
    def loss_fn(params):
        ...
        discriminator_output = state.discriminator_apply_fn({'params': state.discriminator_params}, generated_images)
        loss_adv = adversarial_loss(discriminator_output)
        loss_perceptual = perceptual_loss(state.vgg_apply_fn, state.vgg_params, generated_images, driving_images) 
...
@jax.jit
def validate_step(apply_fn, params, vgg_apply_fn, vgg_params, batch):
    """
    Performs a single validation step.
    Calculates the perceptual loss on the validation set.
    """
    source_images, driving_images, _, _ = batch
    generated_images = apply_fn({'params': params}, source_images, driving_images)
    loss = perceptual_loss(vgg_apply_fn, vgg_params, generated_images, driving_images)
    return loss

def validate(apply_fn, state: TrainState, val_dataloader):
    """
    Performs validation over the entire validation set.
    """
    val_loss = 0.0
    for batch in val_dataloader:
        val_loss += validate_step(apply_fn, state.params, state.vgg_apply_fn, state.vgg_params, batch)
    return val_loss / len(val_dataloader)

def create_train_state(rng, generator, discriminator, vgg, learning_rate):
    """
    Initializes the training state with model parameters and optimizer.
    """
    generator_params = generator.init(rng, jnp.ones((1, IMAGE_SIZE, IMAGE_SIZE, 3)), jnp.ones((1, IMAGE_SIZE, IMAGE_SIZE, 3)))['params']
    discriminator_params = discriminator.init(rng, jnp.ones((1, IMAGE_SIZE, IMAGE_SIZE, 3)))['params']
    vgg_params = vgg.init(rng, jnp.ones((1, IMAGE_SIZE, IMAGE_SIZE, 3)))['params']
    ...
    return TrainState.create(
        ...
        tx=tx,
        discriminator_apply_fn=discriminator.apply,
        vgg_apply_fn=vgg.apply,
...
def train(config):
    ...
    Handles data loading, model initialization, training loop, and validation.
    """
